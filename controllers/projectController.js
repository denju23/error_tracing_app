...
const getProjectById = asyncError(async (req, res) => {
  try {
    ...
    res.status(200).json({
      status: true,
      StatusCode: 200,
      data: { message: "Project Retrived Successfully 😍", result: GetProject },
    });
  } catch (error) {
    errorHandler(res, 400, "Something went wrong🤦");
  }
...
const createProject = asyncError(async (req, res) => {
  try {
    ...
    // console.log(addLoginUserAsMemberRoleOwner,"addLoginUserAsMemberRoleOwner")
    const addProjectInUser = await User.updateOne(
      { _id: user._id },
      { $push: { Projects: getProject } }
    );
    // console.log(addProjectInUser, "addProjectInUser");
