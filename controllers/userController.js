...
const currentUser = asyncError(async (req, res) => {
  try {
    ...
    const getCurrentLoginUser = await User.findById(user._id);
    res.status(200).json({
      status: true,
      statusCode: 200,
    ...
    return errorHandler(res, 400, "somethong went wrong");
  }
});
...
const ForgotPassword = asyncError(async (req, res) => {
  try {
    ...
    const resetPasswordUrl = `http://localhost:3000/api/user/resetpassword/${resetToken}`;
    const htmlTemplate = resetPasswordTemplate(user.name, resetPasswordUrl);
...
const resetpassword = asyncError(async (req, res) => {
  try {
    ...
    if (!getReset) {
      return errorHandler(res, 400, "Token is invalid or expired");
    }
    let getUpdatedData;
